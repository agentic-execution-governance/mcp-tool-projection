...
export default function App() {
  ...
  useEffect(() => {
    if (draft.servers.length === 0) { setPreview(null); return; }
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)", padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          ...
          <span style={{ color: "var(--text-dim)", marginLeft: 8, fontSize: 13 }}>Projection Profile Authoring Tool</span>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="ghost" onClick={() => setDraft(EMPTY_DRAFT)}>Reset</button>
          <button className="primary" onClick={downloadYaml}>↓ Download YAML</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "260px 1fr 280px", overflow: "hidden" }}>
        {/* Left: Servers */}
        <div style={{ borderRight: "1px solid var(--border)", overflow: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
          <h2 style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: 1, color: "var(--text-dim)", margin: 0 }}>Registered Servers</h2>
          <ServerPanel onAddServer={addServer} addedServers={draft.servers.map((s) => s.upstream)} />
        </div>
            ...
            {(["profile", "yaml"] as const).map((tab) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 0, color: activeTab === tab ? "var(--accent)" : "var(--text-dim)",
                  padding: "6px 14px", cursor: "pointer", fontWeight: activeTab === tab ? 700 : 400,
                }}
              >
        ...
        {/* Right: Preview */}
        <div style={{ borderLeft: "1px solid var(--border)", overflow: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
          <h2 style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: 1, color: "var(--text-dim)", margin: 0 }}>
            Effective Tool Surface
            {preview && <span style={{ marginLeft: 8, fontWeight: 400, textTransform: "none", letterSpacing: 0, color: "var(--text-dim)" }}>({preview.effective.length} tools)</span>}
          </h2>
