...
export function PreviewPanel({ preview, loading, error }: Props) {
  ...
  if (!preview)
    return (
      <p style={{ color: "var(--text-dim)", padding: 12, fontSize: 12 }}>
        Add servers to see the effective tool surface.
      </p>
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {collisions.length > 0 && (
        ...
        >
          ...
          <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
            {collisions.map((c) => (
              <span key={c} className="pill collision">
                {c}
              </span>
            ))}
          </div>
          <p style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 6 }}>
            Change collision to <code>prefix</code> or <code>first</code>, or rename the conflicting
            projections.
          </p>
        </div>
      )}
      ...
      {effective.length === 0 && (
        <p style={{ color: "var(--text-dim)", fontSize: 12 }}>
          No tools in the effective surface (all absent?).
        </p>
      )}
        ...
        return (
          <div
            ...
            style={{
              ...
              border: `1px solid ${isCollision ? "var(--red)" : "var(--border)"}`,
              borderRadius: 6,
              padding: "8px 12px",
            }}
          >
